...
def check_base_statistic(data_path,out_dir,start_year=2010,end_year=2025):
    ...
    for domain in domains_cnt:
        domains_cnt[domain]["samples"] = random.sample(domains_cnt[domain]["samples"], min(10,len(domains_cnt[domain]["samples"])))
    sorted_domains = {k:v for k,v in sorted(domains_cnt.items(), key=lambda x: x[1]["count"], reverse=True) if v["count"]>1000}
    ...
    sorted_tags = {k:v for k,v in sorted(tags_cnt.items(), key=lambda x: x[1]["count"], reverse=True)if v["count"]>1000}            
    for tag in sorted_tags:
        sorted_tags[tag]["samples"] = random.sample(sorted_tags[tag]["samples"], min(10,len(sorted_tags[tag]["samples"])))
...
if __name__ == "__main__":
    ...
    stats = check_base_statistic(data_path,out_dir,start_year=2015,end_year=2025)
